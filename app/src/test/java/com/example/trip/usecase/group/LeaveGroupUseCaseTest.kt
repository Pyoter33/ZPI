...
class LeaveGroupUseCaseTest {
    ...
    @Before
    fun setUp() {
        useCase = LeaveGroupUseCase(groupsRepository)
    }

    @Test
    fun `verify group correctly left`() = scope.runTest {
        //given
        coEvery { groupsRepository.leaveGroup(any()) } returns Unit
    ...
    fun `verify result failure Http`() = scope.runTest {
        //given
        coEvery { groupsRepository.leaveGroup(any()) } throws HttpException(Response.error<List<*>>(403, "".toResponseBody()))
    ...
    fun `verify result failure other`() = scope.runTest {
        //given
        coEvery { groupsRepository.leaveGroup(any()) } throws Exception()
