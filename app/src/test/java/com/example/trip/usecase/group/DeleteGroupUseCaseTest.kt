...
class DeleteGroupUseCaseTest {
    ...
    @Before
    fun setUp() {
        useCase = DeleteGroupUseCase(groupsRepository)
    }

    @Test
    fun `verify group correctly deleted`() = scope.runTest {
        //given
        coEvery { groupsRepository.deleteGroup(any()) } returns Unit
    ...
    fun `verify result failure Http`() = scope.runTest {
        //given
        coEvery { groupsRepository.deleteGroup(any()) } throws HttpException(Response.error<List<*>>(403, "".toResponseBody()))
