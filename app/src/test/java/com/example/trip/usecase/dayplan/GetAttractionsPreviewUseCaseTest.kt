...
class GetAttractionsPreviewUseCaseTest {

    private val dispatcher = StandardTestDispatcher()
    private val scope = TestScope(dispatcher)

    private lateinit var useCase: GetAttractionPreviewUseCase
    private val dayPlansRepository: DayPlansRepository = mockk()
    private val attractionDto = AttractionCandidateDto(
        "a",
        "a",
        arrayOf(),
        0.0,
    ...
    @Before
    fun setUp() {
        useCase = GetAttractionPreviewUseCase(dayPlansRepository, "")
    }

    @Test
    fun `verify attraction correctly mapped`() = scope.runTest {
        //given
        coEvery { dayPlansRepository.getAttractionsForQuery(any()) } returns listOf(attractionDto)
        val attraction =
            AttractionPreview(
                "a",
                "a",
    ...
    fun `verify result failure Http`() = scope.runTest {
        //given
        coEvery { dayPlansRepository.getAttractionsForQuery(any()) } throws HttpException(
            Response.error<List<*>>(
                404,
                "".toResponseBody()
            )
    ...
    fun `verify result failure other`() = scope.runTest {
        //given
        coEvery { dayPlansRepository.getAttractionsForQuery(any()) } throws Exception()
