...
class DeleteExpenseUseCaseTest {
    ...
    @Before
    fun setUp() {
        useCase = DeleteExpenseUseCase(financesRepository)
    }
    ...
    fun `verify result failure Http`() = scope.runTest {
        //given
        coEvery { financesRepository.deleteExpense(any(), any()) } throws HttpException(Response.error<List<*>>(403, "".toResponseBody()))
    ...
    fun `verify result failure other`() = scope.runTest {
        //given
        coEvery { financesRepository.deleteExpense(any(), any()) } throws Exception()
