...
class PostTransportUseCaseTest {

    private val dispatcher = StandardTestDispatcher()
    private val scope = TestScope(dispatcher)

    private lateinit var useCase: PostTransportUseCase
    ...
    @Before
    fun setUp() {
        useCase = PostTransportUseCase(transportRepository)
    }

    @Test
    fun `verify transport correctly posted`() = scope.runTest {
        //given
        coEvery { transportRepository.postUserTransport(any(), any()) } returns Unit
    ...
    fun `verify result failure Http`() = scope.runTest {
        //given
        coEvery { transportRepository.postUserTransport(any(), any()) } throws HttpException(Response.error<List<*>>(403, "".toResponseBody()))
    ...
    fun `verify result failure other`() = scope.runTest {
        //given
        coEvery { transportRepository.postUserTransport(any(), any()) } throws Exception()
